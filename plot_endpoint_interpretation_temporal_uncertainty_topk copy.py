...
"""
Endpoint interpretation gold-probability over temporal milestones.

Goal:
    Track whether the probe assigns more probability to the correct
    interpretation over sentence time.

Important:
    This script uses endpoint_interpretation only.

Why:
    We want interpretation probability at specific prefixes:
...
Metrics:
    ...
    gold_margin:
        ...
        positive = probe favors the correct interpretation
        zero     = probe is tied
        negative = probe favors the wrong interpretation
...
Layer handling:
    ...
    3. Keep top-k layers.
    4. Compute p_gold and gold_margin per model x location x milestone x split x layer.
    5. Average across top-k layers within split.
    6. Average across splits.

Outputs:
    endpoint_interpretation_temporal_goldprob_topk_plot/
        endpoint_interpretation_temporal_pgold_topk.png
        endpoint_interpretation_temporal_pgold_topk.pdf
        endpoint_interpretation_temporal_gold_margin_topk.png
        endpoint_interpretation_temporal_gold_margin_topk.pdf
        ...
        topk_layers.csv
        source_layer_split_goldprob.csv
        source_topk_layer_goldprob_rows.csv
        source_split_topk_goldprob.csv
        source_model_topk_goldprob.csv
"""
...
OUTPUT_DIR = Path("endpoint_interpretation_temporal_goldprob_topk_plot")
...
def load_model(model_key, config):
    ...
    #   class 1 = nominalizer
    #
    # p_gold is the probability assigned to the correct interpretation.
    # p_wrong is the probability assigned to the other interpretation.
    df["p_gold"] = df.apply(
        lambda row: (
            float(row["p_class1"])
...
# ==========================================================
# TOP-K LAYER SELECTION BY ACCURACY
# ==========================================================
...
# ==========================================================
# GOLD PROBABILITY AGGREGATION
# ==========================================================

def compute_layer_split_goldprob(rows):
    """
    Compute p_gold and gold_margin per:
        model x location x milestone x split x layer
...
def average_goldprob_topk_layers(layer_split_goldprob, topk_layers):
    """
    Keep only top-k layers.

    Then:
        1. average p_gold and gold_margin across top-k layers within split/milestone
...
def plot_p_gold(model_topk):
    plot_temporal_metric(
        model_topk=model_topk,
        metric_col="mean_topk_p_gold",
        output_filename_stem="endpoint_interpretation_temporal_pgold_topk",
        ...
        title=(
            ...
            "p(gold interpretation), prefix-end representations"
        ),
        y_min=0.35,
        y_max=1.0,
        chance_line=0.5,
...
def plot_gold_margin(model_topk):
    plot_temporal_metric(
        model_topk=model_topk,
        metric_col="mean_topk_gold_margin",
        output_filename_stem="endpoint_interpretation_temporal_gold_margin_topk",
        ...
        title=(
            ...
            "p(gold) - p(wrong), prefix-end representations"
        ),
        y_min=-0.3,
        y_max=1.0,
        chance_line=0.0,
...
def main():
    ...
    print()
    print("Computing layer-split p(gold) and gold margin...")

    layer_split_goldprob = compute_layer_split_goldprob(
        used_rows
    )
    ...
    layer_split_goldprob.to_csv(
        OUTPUT_DIR / "source_layer_split_goldprob.csv",
        index=False,
    )

    topk_layer_goldprob_rows.to_csv(
        OUTPUT_DIR / "source_topk_layer_goldprob_rows.csv",
        index=False,
    )

    split_topk_goldprob.to_csv(
        OUTPUT_DIR / "source_split_topk_goldprob.csv",
        index=False,
    )

    model_topk_goldprob.to_csv(
        OUTPUT_DIR / "source_model_topk_goldprob.csv",
        index=False,
    )

    print()
    print("Plotting p(gold)...")

    plot_p_gold(model_topk_goldprob)

    print("Plotting gold margin...")

    plot_gold_margin(model_topk_goldprob)
    ...
    print()
    print("Main plots:")
    print(OUTPUT_DIR / "endpoint_interpretation_temporal_pgold_topk.png")
    print(OUTPUT_DIR / "endpoint_interpretation_temporal_gold_margin_topk.png")
    print()
    ...
    print(OUTPUT_DIR / "source_model_topk_goldprob.csv")
